...
import { useRef, useState, useEffect, useCallback, useMemo, FormEvent } from "react";
...
import { WagmiProvider, useAccount, useBalance, useSendTransaction, useWaitForTransactionReceipt, useReadContract } from 'wagmi';
...
const MataraLoadingSpinner = ({ size = "md", className = "" }: { size?: "sm" | "md" | "lg", className?: string }) => {
  const sizes = {
    sm: "w-6 h-6",
    md: "w-10 h-10",
    lg: "w-20 h-20"
...
const LoadingScreen = () => (
  ...
  >
    ...
    <div className="space-y-2 text-center">
      ...
      <p className="text-zinc-500 text-[10px] uppercase tracking-widest font-bold">Establishing Secure Connection to BSC...</p>
    </div>
  </motion.div>
);
...
const SectionHeading = ({ subtitle, title }: { subtitle: string, title: string }) => (
  <motion.div 
    ...
    className="mb-12"
  >
    <span className="text-xs font-bold text-amber-400 uppercase tracking-[0.3em]">{subtitle}</span>
    <h2 className="font-headline text-3xl md:text-4xl font-black uppercase mt-2 tracking-tighter">{title}</h2>
  </motion.div>
);
...
const TransactionLog = ({ isDarkMode }: { isDarkMode: boolean }) => {
  ...
  const fetchTxs = useCallback(async () => {
    try {
      const response = await fetch("https://api.dexscreener.com/latest/dex/tokens/0x6844B2e9afB002d188A072A3ef0FBb068650F214");
    ...
    <div className="glass-card overflow-hidden border-white/5">
      <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
        <h3 className="font-headline text-lg font-black uppercase tracking-tight flex items-center gap-2">
          {loading ? <MataraLoadingSpinner size="sm" /> : <RefreshCw className="w-4 h-4 text-amber-400" />}
          Live Transaction Log
        </h3>
        <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Source: BSCScan Protocol</span>
      </div>
          ...
          <tbody className="text-xs font-mono">
            {txs.map((tx, i) => (
              <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                <td className="p-4">
                  <span className={`px-2 py-1 rounded-sm font-bold text-[10px] ${tx.type === 'BUY' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                    {tx.type}
                  </span>
                </td>
                ...
                <td className="p-4">
                  <a href={`https://bscscan.com/tx/${tx.hash}`} target="_blank" rel="noopener noreferrer" className="text-amber-400/60 hover:text-amber-400 transition-colors flex items-center gap-1">
  ...
  return (
    <div className="border-b border-white/5 last:border-0">
      ...
      >
        ...
        >
          <ChevronDown className={`w-5 h-5 ${isOpen ? 'text-amber-400' : 'text-zinc-600'}`} />
        </motion.div>
      </button>
      <motion.div
        initial={false}
...
const blogPosts = [
  {
    ...
    excerpt: "Exploring the vision behind Matara and how we're building a sovereign ecosystem on BSC.",
    date: "Mar 28, 2024",
  ...
  },
  {
    ...
    excerpt: "Understanding the $WKC reflection mechanism and how it benefits long-term $MARS holders.",
    date: "Mar 25, 2024",
  ...
  },
  {
    ...
    excerpt: "How the Pride will shape the future of Matara through our upcoming DAO structure.",
    date: "Mar 20, 2024",
...
const BlogCard = ({ post, onClick }: any) => (
  ...
  >
    ...
    <div className="p-6 space-y-4">
      <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{post.date}</div>
      ...
      <div className="space-y-8">
        ...
        <div className="prose prose-invert max-w-none">
          ...
          <div className="space-y-6 text-zinc-400 leading-relaxed text-lg pt-8">
            <p>
              In the vast expanse of the crypto galaxy, Matara stands as a beacon of sovereignty and strength. 
              As we continue our mission to establish the Marsverse, every milestone achieved is a testament 
              to the unwavering spirit of the Pride.
            </p>
            <p>
              The journey ahead is filled with challenges, but with our community governance structure and 
              the power of $MARS, we are equipped to overcome any obstacle. Our commitment to transparency 
              and security remains our top priority as we scale the ecosystem.
            </p>
            <h3 className="text-white font-headline text-2xl font-black uppercase pt-4">Strategic Expansion</h3>
            <p>
              Our roadmap is not just a plan; it's a declaration of our intent to dominate the decentralized 
              finance space. From Tier-1 CEX listings to the launch of the Marsverse sovereignty, every 
              step is calculated for maximum impact.
            </p>
            <p>
              Stay tuned for more updates from the Intel Center. The roar of the Pride is only getting louder.
            </p>
          </div>
        </div>
...
const AISupport = () => {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'ai', text: string }[]>([
    { role: 'ai', text: "Welcome to the Matara Intel Center. I am your AI guide. How can I assist you in your mission today?" }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: userMsg,
        config: {
          systemInstruction: "You are a helpful AI assistant for Matara ($MARS), a cryptocurrency project on BSC. You are knowledgeable about its ecosystem, tokenomics (690B supply, 1% reflection), and mission. Your tone is heroic, determined, and professional. Keep answers concise.",
        }
      });

      setMessages(prev => [...prev, { role: 'ai', text: response.text || "I am unable to process that request at the moment, warrior." }]);
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'ai', text: "Connection to the Intel Center lost. Please try again later." }]);
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            <div className="p-4 border-b border-white/10 bg-amber-400/10 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className="font-headline text-sm font-black tracking-widest text-amber-400">INTEL CENTER AI</span>
              </div>
...
function AppContent() {
  ...
  const LINKS = {
    ...
    AUDIT: "https://x.com/VB_Audit/status/1966446788988064127?t=uqWs4IJBR8f9IXXqhYPCCA&s=19",
  ...
  useEffect(() => {
    const fetchBnbPrice = async () => {
      try {
        const response = await fetch("https://api.dexscreener.com/latest/dex/tokens/0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"); // WBNB
  ...
  const estimatedMars = useMemo(() => {
    ...
    return (usdVal / marsPrice).toLocaleString(undefined, { maximumFractionDigits: 0 });
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-amber-400 selection:text-black">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-neutral-950/90 py-3 backdrop-blur-xl' : 'bg-transparent py-6'}`}>
                ...
                return (
                  ...
                  >
                    {(() => {
                      if (!connected) {
                        return (
                          <button onClick={openConnectModal} className="text-[10px] font-bold uppercase tracking-widest bg-amber-400 text-black px-4 py-2 rounded-sm hover:bg-amber-300 transition-all">
                            Connect Wallet
                          </button>
                        );
                      }
                      if (chain.unsupported) {
                        return (
                          <button onClick={openChainModal} className="text-[10px] font-bold uppercase tracking-widest bg-red-500 text-white px-4 py-2 rounded-sm">
                            Wrong Network
                          </button>
                        );
                      }
      ...
      <AnimatePresence mode="wait">
        ...
        <section className="relative min-h-screen flex items-center pt-24 md:pt-20 overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,#f7be3310_0%,transparent_50%)]" />
          ...
          <div className="container mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-surface-container-highest border-l-2 border-amber-400">
                <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-amber-400">Mission Protocol Active</span>
              </div>
              
              <h1 className="font-headline text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-tight tracking-tighter">
                Matara ($MARS): <br className="hidden sm:block"/>
                <span className="text-amber-400">The Token of Purpose</span> <br className="hidden sm:block"/>
                on BSC.
              </h1>
              
              <p className="text-on-surface-variant text-base md:text-lg lg:text-xl max-w-lg leading-relaxed font-light">
                Beyond a meme. Beyond speculation. A sovereign ecosystem forged for the modern warrior. Strength, direction, and unyielding conviction.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <a 
                  ...
                  className="solar-flare-gradient text-black px-8 py-4 font-headline font-bold tracking-widest uppercase rounded-sm hover:shadow-[0_0_40px_rgba(247,190,51,0.4)] hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
                >
                  Buy on PancakeSwap
                ...
                <a 
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-surface-container-highest text-white px-8 py-4 font-headline font-bold tracking-widest uppercase rounded-sm border border-white/10 hover:bg-surface-variant hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
                >
                  Join the Pride
              ...
              <div className="pt-8 flex flex-wrap gap-6 md:gap-8 opacity-60">
                {[
                  { icon: Shield, text: "Built on BSC" },
                  { icon: Verified, text: "Fair Launch" },
                  { icon: Zap, text: "Community Governed" }
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-2 hover:opacity-100 transition-opacity">
            ...
            <motion.div 
              ...
              className="relative"
            >
              <div className="absolute inset-0 bg-amber-400/10 blur-[100px] rounded-full scale-75" />
              <img 
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuAV7o7fU6dfo9Cro-BbQqA-g_FUdXeYDMijr0WL5-L_nNXiOd3M5jMKNYG4B2aOonyDKgvHj5VYLloEOb4GZAe07f4xKnqgsfMxSeEC-UcVbTdU_xskivYf91tlVwupaXxS9DffEczRQ7a6ABQuvNMMkazzbipZlU4UKYW0Zm9oPW3LsT1GTZRTlVFLnLmEtrjZLtfz9K1sVNddJHzN77q-yTuab7o4Qc5XK1MSjxkIaOEgm4sxfzLOPr4sl_RDZB1adQ2_VWB_VFrx" 
                alt="Matara Lion Warrior"
                className="relative z-10 w-full max-w-lg mx-auto lg:max-w-none rounded-sm grayscale-[0.2] contrast-[1.1] brightness-[0.9] border-l border-t border-white/10 shadow-2xl"
                referrerPolicy="no-referrer"
                fetchPriority="high"
                decoding="async"
                width={512}
                height={512}
              />
              <div className="absolute -bottom-6 -left-6 glass-card p-4 md:p-6 z-20 hidden sm:block">
                <div className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Live Circulation</div>
                <div className="text-xl md:text-2xl font-headline font-black text-amber-400">
        ...
        {/* Story Section */}
        <section id="story" className="py-20 md:py-32 bg-surface-container-low overflow-hidden">
            ...
            <div className="grid md:grid-cols-12 gap-8 lg:gap-12 items-center">
              ...
              >
                <h2 className="font-headline text-3xl md:text-4xl font-black uppercase tracking-tighter">
                  The Birth of the <br/><span className="text-zinc-500">Lion Warrior</span>
                </h2>
                <div className="space-y-6 text-on-surface-variant text-base md:text-lg leading-relaxed max-w-2xl">
                  <p>
                    <span className="text-white font-bold">Matara</span> means <span className="italic text-amber-400">"Purpose"</span>. In an era where the digital landscape is littered with hollow promises and fleeting noise, we stand as a beacon of architectural intent.
                  </p>
                  <p>
                    We do not build for the next hour; we build for the next age. The Lion represents the strength of our community—a pride that moves with shared conviction. In a market driven by chaos, we build with direction.
                  </p>
                  <div className="pt-4 border-l-2 border-amber-400 pl-4 md:pl-6 py-2 italic font-light hover:bg-white/5 transition-colors">
                    "Every transaction is a heartbeat, every holder is a warrior. This is the sovereignty of the Marsverse."
                  </div>
                  <div className="pt-4">
                    <a 
                      href={LINKS.STORY}
        ...
        <section className="py-20 md:py-32 bg-surface relative">
          <div className="container mx-auto px-6 relative z-10">
            <SectionHeading subtitle="Utility Engine" title="Enter the Marsverse" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="md:col-span-2 lg:col-span-2 glass-card p-6 md:p-10 group hover:bg-surface-container-highest transition-all">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-8 md:mb-12">
                  <div>
                    <h3 className="font-headline text-xl md:text-2xl font-bold text-white mb-2">The Matara Mini-App</h3>
                    <p className="text-on-surface-variant max-w-sm text-sm md:text-base">The command center for your $MARS assets. Fully integrated directly within our ecosystem platforms.</p>
                    <a 
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                  {[
                    ...
                    <li key={i} className="flex items-center gap-3 text-xs md:text-sm font-bold uppercase tracking-wider text-zinc-300 hover:text-white transition-colors cursor-default">
              ...
              <div className="glass-card p-6 md:p-10 group bg-zinc-800/10 border-zinc-700/20">
                ...
                <h3 className="font-headline text-xl md:text-2xl font-bold mb-4">Utility Unlocks</h3>
                <p className="text-xs md:text-sm text-on-surface-variant leading-relaxed">
                  Holding $MARS earns you reflections in $WKC... 1% of buy and sells goes to holders of $MARS
                </p>
              </div>

              <div className="glass-card p-6 md:p-10 group bg-surface-container-highest">
                <h3 className="font-headline text-lg md:text-xl font-bold mb-4 text-zinc-400 group-hover:text-white transition-colors">Live Transaction Feed</h3>
                <div className="space-y-3 overflow-hidden h-40 relative">
                  {[
                    { type: "BUY", amount: "12.4 BNB", time: "2s ago", color: "text-amber-400" },
                    { type: "BUY", amount: "0.8 BNB", time: "14s ago", color: "text-amber-400" },
                    { type: "SELL", amount: "2.1 BNB", time: "45s ago", color: "text-red-400" }
                  ].map((tx, i) => (
                    <div key={i} className="flex justify-between items-center text-[10px] font-mono p-2 bg-neutral-950 border-l border-amber-400 hover:bg-zinc-800 transition-colors">
                      <span className={tx.color}>{tx.type}: {tx.amount}</span>
              ...
              <div className="md:col-span-2 lg:col-span-2 glass-card p-6 md:p-10 flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
                ...
                <div className="flex-1 bg-neutral-950/80 p-6 md:p-8 backdrop-blur-md relative z-10 border border-white/5">
                  <h3 className="font-headline text-xl md:text-2xl font-bold text-white mb-4">Sovereign Governance</h3>
                  <p className="text-on-surface-variant text-xs md:text-sm leading-relaxed">The pride decides. $MARS holders vote on critical ecosystem developments, treasury allocations, and strategic pivots.</p>
        ...
        <section className="py-20 md:py-32 bg-surface-container-low">
          <div className="container mx-auto px-6 max-w-6xl">
            <SectionHeading subtitle="Mission Planning" title="Price Calculator & Swap" />
                ...
                <div className="glass-card p-8 relative overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-6">
                    <div className="flex justify-between items-center">
                      <h3 className="font-headline text-xl font-black uppercase tracking-tight">Swap $MARS</h3>
                      <div className="flex items-center gap-2 px-2 py-1 bg-green-500/10 rounded-full">
                        <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                        <span className="text-[8px] font-bold text-green-500 uppercase tracking-widest">Live DEX</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 bg-neutral-950/50 border border-white/5 rounded-xl">
                        <div className="flex justify-between text-[10px] uppercase font-bold tracking-widest text-zinc-500 mb-2">
                          <span>From</span>
                          <div className="flex items-center gap-2">
                            <span>Balance: {bnbBalance ? `${parseFloat(formatUnits(bnbBalance.value, bnbBalance.decimals)).toFixed(4)} ${bnbBalance.symbol}` : '0.00 BNB'}</span>
                            {bnbBalance && (
                              <button 
                                onClick={() => setBnbInput(formatUnits(bnbBalance.value, bnbBalance.decimals))}
                ...
                <div className="glass-card p-6 border-white/5">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-amber-400/10 flex items-center justify-center shrink-0">
                      {fetchingPrice ? <MataraLoadingSpinner size="sm" /> : <RefreshCw className="w-5 h-5 text-amber-400" />}
                    </div>
                    <div>
                      <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Live $MARS Price</div>
                      <div className="text-lg font-mono font-bold text-white">
                        {marsPrice ? `$${marsPrice.toFixed(10)}` : 'Fetching...'}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Tokenomics Section */}
        <section id="tokenomics" className="py-20 md:py-32 bg-surface-container-low">
          <div className="container mx-auto px-6">
            <div className="text-center mb-16 md:mb-20">
              <h2 className="font-headline text-3xl md:text-4xl font-black uppercase tracking-widest mb-4">$MARS by the Numbers</h2>
              ...
              <div className="text-[10px] md:text-sm font-bold tracking-[0.4em] uppercase text-zinc-500 mt-2">Initial Total Supply</div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {[
                { label: "Liquidity Locked", value: 50, color: "border-amber-400" },
                { label: "Liquidity (Burned)", value: 10, color: "border-white" },
                { label: "Ecosystem Growth", value: 40, color: "border-zinc-600" },
                { label: "Team Reserve", value: 10, color: "border-zinc-400" }
              ].map((item, i) => (
                <motion.div 
                  key={i}
                  initial={{ opacity: 0, y: 20 }}
            ...
            <div className="mt-12 md:mt-16 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 text-center md:text-left">
              {[
                { title: "Transparent", desc: "Full audit visibility on all wallet movements. No blackbox mechanics." },
                { title: "No Hidden Mint", desc: "Contract ownership renounced. The supply is immutable and final." },
                { title: "Clear Allocation", desc: "Every token has a destination mapped on the cosmic roadmap." }
              ].map((item, i) => (
                <div key={i} className="group">
                  <h4 className="font-bold text-white mb-2 uppercase tracking-wider group-hover:translate-x-1 transition-transform">{item.title}</h4>
        ...
        <section id="roadmap" className="py-20 md:py-32 bg-surface">
          <div className="container mx-auto px-6">
            <h2 className="font-headline text-3xl md:text-4xl font-black uppercase text-center mb-16 md:mb-24">The Lion’s Path</h2>
            
            <div className="relative space-y-16 md:space-y-24">
              <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px bg-zinc-800" />
              
              {[
                { phase: "Phase 01", title: "The Awakening", desc: "Smart Contract Deployment, Gitbook Release, Website Launch, Community Formation.", icon: Zap, color: "bg-white text-black" },
                { phase: "Phase 02", title: "The Roar", desc: "PancakeSwap Listing, Marketing Blitz, CoinGecko & CMC Integration, Mini-App Beta.", icon: Megaphone, color: "bg-zinc-700 text-white" },
                { phase: "Phase 03", title: "The Ascent", desc: "Mini-App V1 Launch, First Tier-2 CEX Listings, Global Governance Activation, NFT Staking.", icon: TrendingUp, color: "bg-white text-black" },
                { phase: "Phase 04", title: "The Dynasty", desc: "'Hope Initiatives' Charity Launch, Tier-1 CEX, Cross-chain expansion, Full Marsverse Sovereignty.", icon: Sparkles, color: "bg-zinc-800 text-white" }
              ].map((item, i) => (
                <motion.div 
                  key={i}
                ...
                >
                  <div className={`flex-1 pl-16 md:pl-0 ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                    <div className="text-amber-400 font-headline text-xl md:text-2xl font-black mb-2">{item.phase}</div>
                    <h3 className="text-2xl md:text-3xl font-bold uppercase mb-4">{item.title}</h3>
                    <p className="text-on-surface-variant text-xs md:text-sm max-w-sm md:mx-0">{item.desc}</p>
                  </div>
                  
                  <div className={`absolute left-0 md:relative md:left-auto w-12 h-12 ${item.color} rounded-sm flex items-center justify-center z-10 shrink-0 hover:rotate-45 transition-transform`}>
        ...
        <section className="py-20 bg-surface-container-highest/30 backdrop-blur-md">
          <div className="container mx-auto px-6 grid grid-cols-1 sm:grid-cols-3 gap-12">
            {[
              ...
              { label: "Liquidity Locked", value: 11000, prefix: "$", suffix: "+" }
            ].map((stat, i) => (
        ...
        <section id="blog" className="py-20 md:py-32 bg-surface">
          <div className="container mx-auto px-6 max-w-6xl">
            <SectionHeading subtitle="Galactic Updates" title="The Pride Chronicles" />
        ...
        {/* FAQ Section */}
        <section id="faq" className="py-20 md:py-32 bg-surface-container-low">
          <div className="container mx-auto px-6 max-w-4xl">
            <SectionHeading subtitle="Intel Center" title="Frequently Asked Questions" />
            
            <div className="glass-card p-6 md:p-10">
              {[
                {
                  question: "What is Matara ($MARS)?",
                  answer: "Matara ($MARS) is a sovereign ecosystem built on the Binance Smart Chain (BSC). It represents 'Purpose' and is designed to provide strength, direction, and utility to its community through a range of integrated platforms like the Matara Mini-App."
                },
                {
                  question: "How can I buy $MARS?",
                  answer: "You can purchase $MARS on PancakeSwap by swapping BNB for the $MARS contract address. Ensure you have a compatible wallet like MetaMask or Trust Wallet connected to the BSC network."
                },
                {
                  question: "Is the liquidity locked?",
                  answer: "Yes, 50% of the initial liquidity is locked, and an additional 10% has been burned to ensure long-term stability and security for all holders. You can verify these locks on-chain via our security links."
                },
                {
                  question: "What are the tokenomics?",
                  answer: "The total supply is 690 Billion $MARS. 50% is for locked liquidity, 40% for ecosystem growth, and 10% is held in a team reserve. There is a 1% reflection tax on buys and sells that goes directly back to $MARS holders in $WKC."
                },
                {
                  question: "What is the Matara Mini-App?",
                  answer: "The Matara Mini-App is our ecosystem's command center. It allows users to earn rewards through tasks, track their assets, refer new members, and swap tokens directly within a streamlined interface."
                },
                {
                  question: "How do I join the community?",
                  answer: "You can join our 'Pride' by following us on X (Twitter), joining our Telegram group, or entering our Discord server. Links are available in the footer of this website."
                }
              ].map((faq, i) => (
          ...
          <div className="container mx-auto px-6 max-w-4xl">
            <div className="glass-card p-6 md:p-12 relative overflow-hidden">
              ...
              <div className="relative z-10 text-center space-y-6 md:space-y-8">
                <h2 className="font-headline text-3xl md:text-4xl font-black uppercase">Verify Before You Trust</h2>
                <p className="text-on-surface-variant text-sm md:text-base max-w-xl mx-auto">Security is the foundation of our kingdom. View our verified smart contracts and liquidity locks on-chain.</p>
                
                <div className="space-y-4">
                  <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Official BSC Contract</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 pt-6 md:pt-8">
                  <a href={LINKS.BSCSCAN} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 p-4 bg-surface-container border border-white/5 hover:border-amber-400/40 hover:-translate-y-1 transition-all group">
                    <Search className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest">BSCScan</span>
                  </a>
                  <a href="#" className="flex items-center justify-center gap-3 p-4 bg-surface-container border border-white/5 hover:border-amber-400/40 hover:-translate-y-1 transition-all group">
                    <Lock className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest">Liquidity Lock</span>
                  </a>
                  <a href={LINKS.AUDIT} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 p-4 bg-surface-container border border-white/5 hover:border-amber-400/40 hover:-translate-y-1 transition-all group">
                    <Shield className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest">Audit</span>
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="container mx-auto px-6 max-w-4xl relative z-10">
            <div className="text-center space-y-8">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-400/10 rounded-full border border-amber-400/20">
                ...
                <span className="text-[8px] font-bold text-amber-400 uppercase tracking-widest">Stay Informed</span>
              </div>
              
              <div className="space-y-4">
                <h2 className="font-headline text-3xl md:text-5xl font-black uppercase tracking-tight">Join the Pride Intel</h2>
                <p className="text-on-surface-variant text-sm md:text-base max-w-xl mx-auto">Subscribe to get the latest updates on ecosystem growth, new utility launches, and galactic events.</p>
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="font-headline text-2xl font-black uppercase tracking-tight">Welcome to the Pride!</h3>
                <p className="text-zinc-400 text-sm">Your subscription is confirmed. You'll be the first to receive galactic intel and ecosystem updates.</p>
              </div>
              <button 
                onClick={() => setShowSuccessModal(false)}
      ...
      <AnimatePresence>
        {showSwapConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="flex justify-between items-center">
                <h3 className="font-headline text-xl font-black uppercase tracking-tight">Confirm Swap</h3>
                <button onClick={() => setShowSwapConfirmModal(false)} className="text-zinc-500 hover:text-white transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-white/5">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-green-400 font-bold">&lt; {priceImpact}%</span>
                </div>
                <div className="flex justify-between text-xs">
        ...
        <div className="container mx-auto px-6 md:px-12 py-16 md:py-20 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-start gap-12">
            <div className="space-y-6 max-w-xs">
              <div className="text-lg font-black text-amber-400 font-headline">MATARA</div>
              <p className="text-zinc-500 text-sm leading-relaxed">The sovereign token of the Marsverse. Building a future defined by purpose, strength, and community governance.</p>
            </div>
            
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 md:gap-x-12 gap-y-8">
              <div className="flex flex-col gap-4">
                <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Ecosystem</span>
                <a href={LINKS.WHITEPAPER} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Whitepaper</a>
                <a href={LINKS.AUDIT} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Audit</a>
                <a href={LINKS.BSCSCAN} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">BSCScan</a>
              </div>
              <div className="flex flex-col gap-4">
                <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Social</span>
                <a href={LINKS.TELEGRAM} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Telegram</a>
                <a href={LINKS.X} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">X (Twitter)</a>
                <a href="#" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Discord</a>
              </div>
              <div className="flex flex-col gap-4">
                <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Legal</span>
                <a href="#" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Privacy Policy</a>
                <a href="#" className="text-xs uppercase tracking-widest text-zinc-500 hover:text-amber-400 transition-colors opacity-80 hover:opacity-100">Risk Disclaimer</a>
              </div>
            </div>
          </div>
